...
class Cube:
    def __init__(self, size, center, face_color):
        ...
        self.orientation = np.eye(3)  # represent initial orientation

        # initial empty list
        self.model = None
        self.gen_meshe(size)

        # create a position meshes
        self.create_model()

    def gen_meshe(self, scale: tuple):
        # create central cube meshe
    ...
    def rotate(self, axis, theta):
        # CREATE ROTATION MATRIX BASED ON SPECIFIED AXIS
        if axis == 0:
    ...
    def get_rotation_axis_angle(self):
        # CALCULATE THE ROTATION ANGLE AND AXIS FROM ORIENTATION MATRIX
...
class Rubik:
    ...
    def generate_rubik(self, size):
        ...
        for x in range(3):
            for y in range(3):
                for z in range(3):
                    face_colors = [
                        pr.BLACK if z != 2 else colors[0],  # F
                        pr.BLACK if z != 0 else colors[1],  # B
                        pr.BLACK if x != 2 else colors[2],  # R
                        pr.BLACK if x != 0 else colors[3],  # L
                        pr.BLACK if y != 2 else colors[4],  # T
                        pr.BLACK if y != 0 else colors[5]  # B
                    ]
    ...
    def choose_piece(self, piece, axis_index, level):
        if level == 0 and round(piece[0].center[axis_index], 1) > 0:
            return True
        if level == 1 and round(piece[0].center[axis_index], 1) == 0:
            return True
        if level == 2 and round(piece[0].center[axis_index], 1) < 0:
            return True
        return False
    ...
    def handle_rotation(self, rotation_queue, animation_step=None):
        # check if there is a request and if not rotating
        if rotation_queue and not self.is_rotating:

            # GET NEXT ROTATION AXIS AND LEVEL
            ...
            # RESET ROTATION ANGLE AT START OF A NEW ROTATION
            self.rotation_angle = 0

            # SET ROTATING TO TRUE AND START ROTATION
        ...
        if self.is_rotating:
            if (self.rotation_angle != self.target_rotation):
                ...
                # STOP ROTATING WHEN TARGET ROTATION IS READCHED
                self.is_rotating = False

                if animation_step is not None:
                    animation_step += 1
                # print('increment animation step',animation_step)
